...
key_repository = ClaudeAPIKeyRepository(settings.CLAUDE_KEY_DB_PATH)
impl: AbstractClaudeChatAPI = ClaudeChatImplementation(key_repository=key_repository)
...
# async def auth_callback(request: Request, code: str):
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={"detail": f"OAuth processing failed: {e!s}"}
        )


@app.post("/users/me/claude-key", status_code=status.HTTP_204_NO_CONTENT, tags=["Authentication"])
async def upsert_claude_api_key(
    payload: ClaudeKeyRequest,
    current_user_id: str = Depends(get_current_user_id),
) -> Response:
    """Store or update the authenticated user's Claude API key.
    """
    sanitized_key = payload.api_key.strip()
    if not sanitized_key:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="API key must not be empty.",
        )
    key_repository.set_api_key(current_user_id, sanitized_key)
...
async def send_chat_message(
    ...
    try:
        ...
        ai_response = impl.send_message(
            prompt=chat_request.prompt,
            user_id=current_user_id
        )
        return ai_response
    except MissingClaudeAPIKeyError as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(exc),
        ) from exc
